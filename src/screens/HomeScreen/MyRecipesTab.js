  ...
  useFocusEffect(
    useCallback(() => {
      const fetchMyRecipes = async () => {
        setLoading(true);
        try {
          const { data } = await apiClient.get('/recipes/myrecipes', {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => navigation.navigate('RecipeDetail', { recipeId: item._id })} style={styles.recipeCard}>
            ...
            <View style={styles.recipeContent}>
              ...
              <Text style={styles.recipeUser}>Por: {item.user?.username || 'Tú'}</Text>
        ...
        style={styles.list}
        ListEmptyComponent={<Text style={styles.emptyText}>Aún no has creado ninguna receta. ¡Crea la primera!</Text>}
      />
    </View>
  );
};
